...
const Search = ({ updateAuthenticationStatus }) => {
    ...
    // Function to render park details or park name button
    const renderParkInfo = (park) => {
        if (parkDetails && parkDetails.fullName === park.fullName) {
            return (
                <div>
                    <h3><a href="{parkDetails.url}" target="_blank">{parkDetails.fullName}</a></h3>
                    <p>
                        <strong>Location:</strong> {parkDetails.addresses[0].line1}, {parkDetails.addresses[0].city}, {parkDetails.addresses[0].stateCode}
                    </p>
                    <div>
            ...
            return (
                <button id="expand" onClick={() => handleParkClick(park.parkCode)}>
